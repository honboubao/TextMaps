...
if __name__ == "__main__":
    ...
    # # load pages
    # pages_path = os.path.join('../data_shops/page_sets/', prefix+'.txt')
    # with open(pages_path,'r') as f:
    #     pages = [line.split('\t')[0] for line in f.readlines()]
    ...
    #     # prepare paths
    #     dom_path = os.path.join('../data_shops/labeled_dom_trees/', page+'.json')
    #     page_image_path = os.path.join('../data_shops/images/', page+'.jpeg')
    ...
    #     for i in range(len(labels)):
    #         label = labels[i]
    #         ax = fig.add_subplot(number_of_pages,3,(page_ind*3)+i+1)
    #         ax.imshow(getPatch(im,labeled[label]))
    ...
    # TODO:
    #  - Rozdeli na tri faze (prepinac v paremetru), prepare patches, remove mislabbeled, remove patches
